...
add_filter('acf/save_post', 'convert_dept_to_standard_wp_meta', 20);
 
function convert_dept_to_standard_wp_meta($post_id) {
  ...
  if (have_rows('display_order', $post_id)) {
    while (have_rows('display_order', $post_id)) {
      the_row();
       
      // get the value of this row
      $deptarray = get_sub_field('department');
		  $deptorder = get_sub_field('display_order_within_department');

		$dept = $deptorder ._. $deptarray->slug;
      ...
      add_post_meta($post_id, $meta_key, $dept, false);
       
      // add it to the values we've already saved
      $saved_values[$dept] = $dept;
