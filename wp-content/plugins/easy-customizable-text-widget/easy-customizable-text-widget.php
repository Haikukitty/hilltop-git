...
define('WPS_GFW_PATH', plugin_dir_path(__FILE__));
define('WPS_GFW_URL', plugins_url('', __FILE__));
...
class wps_gfw {
  ...
  static function premium_subscribe() {
    ...
    if (!empty($_POST)) {
      ...
      $data = array('email_address' => $_POST['email'], 'status' => 'pending');
  ...
  static function get_fonts() {
	...
	if (empty($fonts) || !$fonts) {
		$fonts = file_get_contents(plugins_url('fonts/fonts.txt', __FILE__));
	}
