...
if (get_option('overall_plugin_tour') && get_option('overall_plugin_tour')=='true') {
    ...
    if ($tab == 'rolemapping') {
        /*$pointers['miniorange-ldap-fetch-groups'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Group Container')),
            'content' => sprintf('<p>%s</p>', esc_html__('Enter the container path to fetch the group names.')),
            'anchor_id' => '#mo_ldap_groups_search_base_ldap',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );*/
        $pointers['miniorange-default-role-mapping'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Group Attribute')),
            'content' => sprintf('<p>%s</p>', esc_html__('Select the default role which should be assigned to new users.')),
            'anchor_id' => '#default_role_value',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        $pointers['miniorange-enable-role-mapping'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Enable Role Mapping')),
            'content' => sprintf('<p>%s</p>', esc_html__('Enable this option to assign the default role to the users')),
            'anchor_id' => '#enable_role_mapping_pointer',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        /*$pointers['miniorange-groups-mapping'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Group ')),
            'content' => sprintf('<p>%s</p>', esc_html__('Enter distinguished name of the group to map to WP role.')),
            'anchor_id' => '#selected_group',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        $pointers['miniorange-groups-value-mapping'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Group ')),
            'content' => sprintf('<p>%s</p>', esc_html__('Role which will be assigned to the users of mapped Directory group.')),
            'anchor_id' => '#selected_group_value',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        $pointers['miniorange-attribute-mapping'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Group Attribute')),
            'content' => sprintf('<p>%s</p>', esc_html__('Enter the ldap attribute name which contains users groups.')),
            'anchor_id' => '#role_mapping_form_ldap',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        $pointers['miniorange-role-mapping'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Test Role Mapping Configuration')),
            'content' => sprintf('<p>%s</p>', esc_html__('It will help you to test the Directory group to WordPress role mapping you\'ve configured. ')),
            'anchor_id' => '#mo_rolemap_ldap_username',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );*/

        $pointers['role-mapping-ldap-tour'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Click when you need me!')),
            'content' => sprintf('<p>%s</p>', esc_html__('Revisit tour')),
            'anchor_id' => '#role-map-service-restart-tour',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
    ...
    if ($tab == 'attributemapping') {
        $pointers['minorange-ldap-attr-mail-map'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__(' Set Attribute')),
            'content' => sprintf('<p>%s</p>', esc_html__('Set the ldap attribute which contains email of the user. e.g. mail')),
            'anchor_id' => '#ldap_intranet_attribute_mail_name',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        $pointers['miniorange-test-attr-config'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Test Attribute configuration')),
            'content' => sprintf('<p>%s</p>', esc_html__('This will fetch and show the LDAP attributes of the user configured above')),
            'anchor_id' => '#attribiteconfigtest_ldap',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
        $pointers['miniorange-redirection-ldap-restart-tour'] = array(
            'title' => sprintf('<h3>%s</h3>', esc_html__('Click when you need me!')),
            'content' => sprintf('<p>%s</p>', esc_html__('Revisit tour')),
            'anchor_id' => '#att-map-service-restart-tour',
            'edge' => 'left',
            'align' => 'left',
            'where' => array('toplevel_page_mo_ldap_local_login') // <-- Please note this
        );
