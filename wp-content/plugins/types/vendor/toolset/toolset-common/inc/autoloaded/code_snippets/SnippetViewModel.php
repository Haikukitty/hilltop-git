...
class SnippetViewModel {
	...
	public function apply_array( $model ) {
		...
		if( $this->snippet->is_editable() ) {
			...
			if( $model_code !== $this->snippet->get_code() ) {
				...
				if( $is_code_updated ) {
					...
					$results->add( false, sprintf( __( 'Unable to update the snippet file "%s".', 'wpcf' ), $this->snippet->get_file_subpath() ) );
				}
		...
		if( ! $results->has_errors() ) {
			$results->add( true, sprintf( __( 'Snippet "%s" has been updated.', 'wpcf' ), $this->snippet->get_slug() ) );
		}
