	...
	public function run( SnippetViewModel $view_model, $model ) {
		...
		if( 'true' === toolset_getarr( $model, 'runNow' ) ) {
			...
			if( $run_result->is_success() ) {
				$results->add( true, __( 'The snippet has been re-run without errors.', 'wpcf' ) );
			} else {
				$results->add( false, __( 'There was error when trying to re-run the snippet:', 'wpcf' ) . $run_result->get_message() );
			}
		}
