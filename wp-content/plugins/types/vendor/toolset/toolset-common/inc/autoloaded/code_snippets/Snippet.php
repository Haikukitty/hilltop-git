	...
	public function run() {
		...
		if( ! $this->files->is_file( $path ) ) {
			return new \Toolset_Result( false, sprintf(
				__( 'The code snippet file "%s" was not found or is not a regular file.', 'wpcf' ),
				$path
			) );
		}
		...
		if( ! $this->files->is_readable( $path ) ) {
			return new \Toolset_Result( false, sprintf(
				__( 'The code snippet file "%s" is not readable.', 'wpcf' ),
				$path
			) );
		}
		...
		if( ! empty( $include_output ) ) {
			$results->add( false, sprintf(
				__( 'Unexpected output of the code snippet: %s', 'wpcf' ),
		...
		return new \Toolset_Result( true, sprintf( __( 'Executed code snippet "%s".', 'wpcf'), $this->get_slug() ) );
	}
