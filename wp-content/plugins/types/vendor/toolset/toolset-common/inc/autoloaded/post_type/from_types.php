	...
	public function can_be_used_as_intermediary( $skip_check_for_existing_intermediary = false, $skip_check_for_relationship_involvment = false ) {
		if( ! $skip_check_for_existing_intermediary && $this->is_intermediary() ) {
			return new Toolset_Result(
				false, sprintf( __( 'The post type "%s" cannot be used as intermediary in another relationship, because it is already used as one.', 'wpcf'), $this->get_slug() )
			);
		}
		if( ! $skip_check_for_relationship_involvment && $this->is_involved_in_relationship() ) {
			return new Toolset_Result(
				false, sprintf( __( 'The post type "%s" cannot be used as intermediary in a relationship, because it is already involved in another relationship.', 'wpcf'), $this->get_slug() )
			);
		}
		if( $this->is_builtin() ) {
			return new Toolset_Result(
				false, sprintf( __( 'The post type "%s" cannot be used as intermediary in a relationship, because it is a built-in post type.', 'wpcf'), $this->get_slug() )
			);
		}
		if( $this->is_repeating_field_group() ) {
			return new Toolset_Result(
				false, sprintf( __( 'The post type "%s" cannot be used as intermediary in a relationship, because it represents a repeatable field group.', 'wpcf'), $this->get_slug() )
			);
		}
