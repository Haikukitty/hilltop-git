	...
	public function create_snippet( $path, $options ) {
		if( ! $this->snippet_explorer->is_in_supported_directory( $path ) ) {
			throw new SnippetCreationException(
				sprintf(
					__( 'The snippet file "%s" is not in a supported directory "%s".', 'wpcf' ),
		...
		if( ! $this->files->is_file( $path ) ) {
			throw new SnippetCreationException(
				sprintf( __( 'The snippet file "%s" doesn\'t exist or isn\'t a regular file.', 'wpcf' ), sanitize_text_field( $path ) )
			);
		}
