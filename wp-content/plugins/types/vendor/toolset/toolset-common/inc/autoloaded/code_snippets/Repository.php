...
class Repository {
	...
	public function rename_snippet_slug( Snippet $snippet, $new_slug ) {
		...
		if ( empty( $new_slug ) ) {
			return new \Toolset_Result( false, __( 'A snippet slug cannot be empty.', 'wpcf' ) );
		}

		if ( sanitize_title( str_replace( '.', '-', $new_slug ) ) !== str_replace( '.', '-', $new_slug ) ) {
			return new \Toolset_Result( false, __( 'The new slug value is not valid. Only lowercase letters, numbers, underscores, dots and dashes are accepted.', 'wpcf' ) );
		}

		if ( array_key_exists( $new_slug, $this->snippets ) ) {
			return new \Toolset_Result( false, sprintf( __( 'Slug "%s" is already being used. Please choose a unique value.', 'wpcf' ), $new_slug ) );
		}
	...
	public function insert( Snippet $snippet ) {
		...
		if ( array_key_exists( $snippet->get_slug(), $this->snippets ) ) {
			return new \Toolset_Result( false, sprintf( __( 'Slug "%s" is already being used. Please choose a unique value.', 'wpcf' ), $snippet->get_slug() ) );
		}
		...
		return new \Toolset_Result( true, __( 'New snippet has been inserted.', 'wpcf' ) );
	}
