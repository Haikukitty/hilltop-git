...
class SettingsTab {
	...
	public function render_add_new_button() {
		return '<a class="add-new-h2" data-bind="click: onAddNew">' . __( 'Add new', 'wpcf' ) . '</a>';
	}
	...
	private function build_js_data() {
		return array(
			...
			'strings' => array(
				...
				'updateResults' => array(
					'error' => __( 'There was an error when updating snippets.', 'wpcf' ),
				),
				'dialogs' => array(
					'addNewDialog' => array(
						'handle' => self::ADD_NEW_DIALOG_HANDLE,
						'title' => __( 'Create a new code snippet', 'wpcf' ),
						'cancel' => __( 'Cancel', 'wpcf' ),
						'create' => __( 'Create', 'wpcf' ),
					),
					'deleteDialog' => array(
						'handle' => self::DELETE_DIALOG_HANDLE,
						'title' => __( 'Delete a code snippet', 'wpcf' ),
						'cancel' => __( 'Cancel', 'wpcf'),
						'delete' => __( 'Delete', 'wpcf' )
					)
				),
				'runMode' => array(
					SnippetOption::RUN_ONCE => __( 'Run once', 'wpcf' ),
					SnippetOption::RUN_ON_DEMAND => __( 'On demand', 'wpcf' ),
					SnippetOption::RUN_ALWAYS => __( 'Always', 'wpcf' )
				),
				'runContexts' => array(
					RequestMode::ADMIN => __( 'WordPress admin', 'wpcf' ),
					RequestMode::AJAX => __( 'AJAX calls', 'wpcf' ),
					RequestMode::FRONTEND => __( 'Front-end', 'wpcf' ),
					'everywhere' => __( 'Everywhere', 'wpcf' ),
					'nowhere' => __( 'Nowhere', 'wpcf' ),
				)
			),
