...
class Toolset_Ajax_Handler_Get_Cred_Form_Block_Preview extends Toolset_Ajax_Handler_Abstract {
	...
	function process_call( $arguments ) {
		...
		if ( 0 === $cred_form_id ) {
			$this->ajax_finish( array( 'message' => __( 'CRED form ID not set.', 'wp-cred' ) ), false );
		} else {
			$cred_form = get_post( $cred_form_id );

			if ( null === $cred_form ) {
				$this->ajax_finish( array( 'message' => sprintf( __( 'Error while retrieving the CRED form preview. The selected CRED form (ID: "%s") was not found.', 'wp-cred' ), $cred_form_id ) ), false );
			} else {
